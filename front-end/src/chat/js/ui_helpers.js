...
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status){
  ...
  var delivered = '<img class="icon-small" src="src/chat/images/delivered.jpg" alt="" id="delivered_'+messageId+'">';
  var read = '<img class="icon-small" src="src/chat/images/read.jpg" alt="" id="read_'+messageId+'">';
...
function buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage) {
  ...
  var dialogHtml =
      '<a href="javascript:void(0)" class="list-group-item inactive" id='+'"'+dialogId+'"'+' onclick="triggerDialog('+"'"+dialogId+"'"+')">'+
...
function buildUserHtml(userLogin, userId, isNew) {
  var userHtml = "<a href='javascript:void(0)' id='" + userId;
  if(isNew){
    userHtml += "_new'";
  }else{
    userHtml += "'";
